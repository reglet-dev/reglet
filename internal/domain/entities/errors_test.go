...
func TestPluginNotFoundError_ErrorMessage(t *testing.T) {
	ref := values.NewPluginReference("ghcr.io", "example", "plugins", "my-plugin", "1.0.0")

	notFoundErr := &PluginNotFoundError{Reference: ref}
...
func TestPluginNotFoundError_ErrorIs(t *testing.T) {
	...
	assert.True(t, errors.Is(notFoundErr, ErrPluginNotFound))

	// Should not match other sentinel errors
	assert.False(t, errors.Is(notFoundErr, ErrIntegrityCheckFailed))
}
...
func TestPluginNotFoundError_ErrorAs(t *testing.T) {
	...
	var typedErr *PluginNotFoundError
	...
	assert.True(t, errors.Is(wrappedErr, ErrPluginNotFound))
}

func TestIntegrityError_ErrorMessage(t *testing.T) {
	expected, err := values.NewDigest("sha256", "abc123")
	require.NoError(t, err)

	actual, err := values.NewDigest("sha256", "def456")
	require.NoError(t, err)
...
func TestIntegrityError_ErrorIs(t *testing.T) {
	...
	// Should support errors.Is() with sentinel error
	assert.True(t, errors.Is(integrityErr, ErrIntegrityCheckFailed))

	// Should not match other sentinel errors
	assert.False(t, errors.Is(integrityErr, ErrPluginNotFound))
}
...
func TestErrorPatterns(t *testing.T) {
	...
	t.Run("Pattern 1: errors.Is() for quick type checks", func(t *testing.T) {
		...
		if errors.Is(wrappedErr, ErrPluginNotFound) {
